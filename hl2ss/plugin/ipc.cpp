...
static HANDLE g_semaphore_so_data; // CloseHandle
...
static CRITICAL_SECTION g_lock_so_data; // DeleteCriticalSection
static std::queue<MQ_MSG> g_queue_si;
static std::queue<uint32_t> g_queue_so;
static std::queue<MQ_MSG> g_queue_so_data;
...
// Testing
static DWORD WINAPI MQ_EntryPoint_SendData(void* param)
{
	SOCKET clientsocket = *((SOCKET*)param);
	int status;
	MQ_MSG msg;
	...
	do
	{
		...
		status = send(clientsocket, (char*)&msg.command, sizeof(msg.command), 0);
		if (status != sizeof(msg.command)) { break; }
		status = send(clientsocket, (char*)&msg.size, sizeof(msg.size), 0);
		if (status != sizeof(msg.size)) { break; }
		if (msg.size > 0)
		{
...
void MQ_SO_Push_Data(uint32_t command, uint8_t* data, uint32_t size)
{
	...
	memcpy(msg.data, data, size);
	{
		...
		ReleaseSemaphore(g_semaphore_so_data, 1, NULL);
	}
}
...
static void MQ_Procedure(SOCKET clientsocket)
{
	...
	g_semaphore_so_data = CreateSemaphore(NULL, 0, LONG_MAX, NULL);
	...
	threads[2] = CreateThread(NULL, 0, MQ_EntryPoint_SendData, &clientsocket, 0, NULL);
	...
	CloseHandle(g_semaphore_so_data);
...
void MQ_Initialize()
{
	...
	InitializeCriticalSection(&g_lock_so_data);
...
void MQ_Cleanup()
{
	...
	DeleteCriticalSection(&g_lock_so_data);
